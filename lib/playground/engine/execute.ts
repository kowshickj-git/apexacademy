...
export class AVRRunner {
  ...
  /** APEX: pause flag (loop stays scheduled but executes no cycles). */
  paused = false;
  private lastTime = 0;
  ...
  execute(callback: (cpu: CPU) => void) {
    ...
    // APEX: pause support — keep the rAF loop alive but skip cycle execution.
    if (this.paused) {
  ...
  /** APEX: pause without tearing down CPU state. */
  pause() {
    this.paused = true;
  }

  /** APEX: resume from pause; reset the timing anchor so we don't fast-forward. */
  resume() {
    if (this.paused) {
      this.paused = false;
      this.lastTime = 0;
