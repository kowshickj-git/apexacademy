...
export default function MultimeterComplete({ xp, onClose }: Props) {
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className="w-full max-w-sm rounded-3xl border overflow-hidden text-center relative z-10"
        style={{ background: "rgba(18,18,27,0.98)", borderColor: "rgba(167,139,250,0.3)" }}
      >
        {/* Header */}
        <div
          className="px-6 pt-8 pb-6 border-b border-white/5"
          style={{ background: "linear-gradient(to bottom, rgba(167,139,250,0.12), transparent)" }}
        >
          <div
          ...
          <h2 className="text-2xl font-black mb-1 text-white">Course Milestone!</h2>
          <p className="text-sm text-white/45">Multimeter Fundamentals — L10</p>
          <div
            className="mt-3 text-xs font-semibold px-3 py-1.5 rounded-full inline-block"
            style={{ background: "rgba(167,139,250,0.12)", color: "#A78BFA" }}
          >
            You&apos;ve completed all 10 Electronics Fundamentals lessons!
          </div>
        </div>
        ...
        <div className="px-6 py-5 border-b border-white/5">
          <div className="flex items-center justify-center gap-3 mb-4">
            ...
            <span
              ...
              style={{
                background: "linear-gradient(135deg, #A78BFA, #0EA5E9, #10B981)",
          ...
          <div className="mb-2">
            ...
            <div className="h-2 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.06)" }}>
              <motion.div
                ...
                style={{ background: "linear-gradient(to right, #8B5CF6, #A78BFA, #10B981)" }}
              />
            </div>
          </div>
        </div>
        ...
        <div className="px-6 py-4 border-b border-white/5">
          ...
          <div className="grid grid-cols-4 gap-2">
            {[
              { icon: "⚡", label: "Voltmeter" },
              { icon: "🔌", label: "Ammeter" },
              { icon: "Ω", label: "Ohmmeter" },
              { icon: "🔊", label: "Continuity" },
            ].map(({ icon, label }) => (
              <div
                key={label}
                className="flex flex-col items-center gap-1 px-2 py-3 rounded-xl border"
                style={{ borderColor: "rgba(167,139,250,0.25)", background: "rgba(167,139,250,0.08)" }}
              >
                <span className="text-lg">{icon}</span>
                <span className="text-[9px] text-center" style={{ color: "rgba(196,181,253,0.6)" }}>
                  {label}
                </span>
              </div>
            ))}
        ...
        <div className="px-6 py-5 space-y-3">
          ...
          >
            ...
            <div className="text-left">
              <div className="flex items-center gap-2">
                <p className="text-xs font-bold text-white/60">Next: Lesson 11</p>
                <span
              ...
              <p className="text-sm font-bold text-white/40">Series Circuits</p>
            </div>
          </div>
          ...
          <Link
            href="/index.html"
            className="block w-full py-2.5 rounded-2xl font-semibold text-sm text-center border transition-colors"
