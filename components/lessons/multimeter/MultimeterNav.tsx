...
export default function MultimeterNav({ xp, milestones, onMenuClick }: Props) {
  ...
  const milestoneList = [
    ...
    milestones.voltageUsed,
    milestones.currentUsed,
    milestones.resistanceUsed,
    milestones.continuityUsed,
  ...
  return (
    ...
    >
      <div className="max-w-5xl mx-auto px-3 h-14 flex items-center gap-2">
        ...
        {/* Milestone pips: Read, Voltage, Current, Resistance, Continuity, Quiz */}
        <div
          className="hidden sm:flex items-center gap-1"
          title={`${completedCount}/6 milestones`}
        >
          {["Read", "Voltage", "Current", "Resistance", "Continuity", "Quiz"].map((label, i) => (
            <div
              key={label}
        ...
        <button
          onClick={onMenuClick}
          className="ml-1 text-white/30 hover:text-white/60 transition-colors p-1"
        >
