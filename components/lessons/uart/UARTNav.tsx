...
export default function UARTNav({ xp, onMenuClick }: Props) {
  const [scrollPct, setScrollPct] = useState(0);

  useEffect(() => {
    const f = () => {
      const el = document.documentElement;
      setScrollPct(el.scrollHeight - el.clientHeight > 0 ? (el.scrollTop / (el.scrollHeight - el.clientHeight)) * 100 : 0);
    };
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-1.5 text-xs min-w-0">
        ...
        <span className="font-semibold truncate" style={{ color: COLOR }}>UART Fundamentals</span>
      </div>

      {/* Lesson badge */}
      <span
        className="hidden sm:inline-block text-xs font-mono px-2 py-0.5 rounded-full flex-shrink-0"
        style={{ background: "rgba(16,185,129,0.1)", color: "rgba(16,185,129,0.7)", border: "1px solid rgba(16,185,129,0.2)" }}
      >
        Embedded Systems · L23
      </span>
      ...
      <div
        ...
        style={{ background: "rgba(16,185,129,0.1)", border: "1px solid rgba(16,185,129,0.22)", color: COLOR }}
      >
      ...
      <Link
        ...
        style={{ color: COLOR, border: `1px solid rgba(16,185,129,0.3)`, background: "rgba(16,185,129,0.07)" }}
      >
        I2C →
      </Link>
      ...
      <motion.div
        ...
        style={{ width: `${scrollPct}%`, background: `linear-gradient(90deg, ${COLOR}, #34D399)` }}
      />
    </nav>
  );
}
