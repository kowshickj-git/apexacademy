...
const lessons = [
  ...
  { num: "03", title: "Resistance",  href: "/electronics/resistance",    status: "active"    },
  { num: "04", title: "Resistors",   href: "/electronics/resistors",     status: "available" },
...
export default function LearningPath() {
  return (
    <div className="p-4">
      <p className="text-[10px] text-white/30 uppercase tracking-widest font-semibold mb-3 font-mono">
        Learning Path
      </p>
      ...
      <div className="mb-4">
        ...
        <div className="h-1.5 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.06)" }}>
          <div
            ...
            style={{ width: "30%", background: "linear-gradient(to right, #10B981, #0EA5E9)" }}
          />
        </div>
      </div>
      ...
      <div className="space-y-0.5">
        {lessons.map((lesson) => {
          if (lesson.status === "locked") {
            return (
              <div key={lesson.num} className="flex items-center gap-2.5 px-2.5 py-2 rounded-lg cursor-not-allowed select-none">
                ...
                <span className="text-[11px] text-white/20">L{lesson.num} · {lesson.title}</span>
              </div>
            );
          }
          if (lesson.status === "available") {
            return (
              <Link key={lesson.num} href={lesson.href} className="flex items-center gap-2.5 px-2.5 py-2 rounded-lg hover:bg-white/4 transition-colors group">
                ...
                <span className="text-[11px] text-white/38 group-hover:text-white/55 transition-colors">L{lesson.num} · {lesson.title}</span>
              </Link>
            );
          }
          if (lesson.status === "done") {
            return (
              <Link key={lesson.num} href={lesson.href} className="flex items-center gap-2.5 px-2.5 py-2 rounded-lg hover:bg-white/4 transition-colors">
                ...
                <span className="text-[11px] text-white/50">L{lesson.num} · {lesson.title}</span>
              </Link>
            );
          }
          return (
            <div key={lesson.num} className="flex items-center gap-2.5 px-2.5 py-2 rounded-lg border border-primary/15" style={{ background: "rgba(16,185,129,0.07)" }}>
              <div className="w-5 h-5 rounded-full bg-primary flex items-center justify-center flex-shrink-0">
                <div className="w-1.5 h-1.5 rounded-full bg-white" />
              </div>
              <span className="text-[11px] text-primary font-semibold">L{lesson.num} · {lesson.title}</span>
            </div>
          );
        })}
      </div>
