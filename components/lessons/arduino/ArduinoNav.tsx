...
const PIPS=[{key:"lessonRead",label:"Read"},{key:"blinkUsed",label:"Blink"},{key:"digitalReadUsed",label:"Digital"},{key:"analogUsed",label:"Analog"},{key:"serialUsed",label:"Serial"},{key:"projectUsed",label:"Project"},{key:"quizPassed",label:"Quiz"}] as const;

export default function ArduinoNav({ xp, milestones, onMenuClick }: Props) {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-30 h-14 flex items-center px-4 gap-3" style={{background:"rgba(5,5,7,0.92)",backdropFilter:"blur(16px)",borderBottom:"1px solid rgba(255,255,255,0.06)"}}>
      <Link href="/" className="flex-shrink-0">
        <svg width="28" height="28" viewBox="0 0 28 28" fill="none"><polygon points="14,3 25,22 3,22" fill="#059669" opacity="0.9"/><polygon points="14,8 22,21 6,21" fill="#050507"/><polygon points="14,11 19,20 9,20" fill="#059669" opacity="0.5"/></svg>
      </Link>
      ...
      <div className="flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-black flex-shrink-0" style={{background:"rgba(5,150,105,0.1)",border:"1px solid rgba(5,150,105,0.22)",color:"#059669"}}>
        <svg width="10" height="10" viewBox="0 0 12 12" fill="#059669"><polygon points="6,1 7.5,4.5 11,5 8.5,7.5 9,11 6,9.5 3,11 3.5,7.5 1,5 4.5,4.5"/></svg>
        {xp} XP
      </div>
      ...
      <motion.div className="absolute bottom-0 left-0 h-0.5" style={{width:`${scrollPct}%`,background:"linear-gradient(90deg, #059669, #10B981)"}}/>
    </nav>
  );
}
