...
export default function PullupNav({ xp, milestones, onMenuClick }: Props) {
  ...
  const pipData = [
    ...
    { label: "Floating", done: milestones.floatingUsed },
    { label: "PullUp", done: milestones.pullupUsed },
    { label: "PullDown", done: milestones.pulldownUsed },
    { label: "Arduino", done: milestones.arduinoInputUsed },
    { label: "Debugger", done: milestones.debuggerUsed },
    ...
    >
      <div className="max-w-5xl mx-auto px-3 h-14 flex items-center gap-2">
        ...
        <div className="hidden md:flex items-center gap-1.5 text-xs">
          ...
          <span className="font-medium" style={{ color: "#F59E0B" }}>Pull-Up &amp; Pull-Down</span>
        </div>

        <span className="text-[10px] text-white/20 font-mono hidden lg:block">L14 of 36</span>
        ...
        <div className="hidden sm:flex items-center gap-1" title={`${pipData.filter(p => p.done).length}/7 milestones`}>
          {pipData.map((pip) => (
            <div
              ...
              style={{ background: pip.done ? "#F59E0B" : "rgba(255,255,255,0.1)" }}
              title={pip.label}
            />
          ))}
        </div>
        ...
        >
          ...
          <span className="text-xs font-bold tabular-nums" style={{ color: "#F59E0B" }}>{xp} XP</span>
        </div>
        ...
        <Link
          ...
          style={{ background: "rgba(245,158,11,0.09)", borderColor: "rgba(245,158,11,0.22)", color: "#F59E0B" }}
        >
      ...
      {/* Scroll progress bar — amber gradient */}
      <div className="h-[2px] bg-white/4">
        <motion.div
          className="h-full origin-left"
          style={{ scaleX: scrollProgress / 100, background: "linear-gradient(to right, #F59E0B, #D97706)" }}
        />
      </div>
    </header>
  );
