...
const lessons = [
  ...
  {num:"19",title:"Logic Gates",href:"/electronics/logic-gates",status:"active"},
...
export default function LearningPath({ milestones }: Props) {
  ...
  return (
    <div className="p-4">
      <div className="mb-4">
        <div className="flex justify-between text-[10px] text-white/30 mb-1 font-mono"><span>Overall progress</span><span>19/20 = 95%</span></div>
        <div className="h-1.5 rounded-full overflow-hidden" style={{background:"rgba(255,255,255,0.06)"}}><div className="h-full rounded-full" style={{width:"95%",background:"linear-gradient(90deg,#84CC16,#A3E635)"}}/></div>
      </div>
      <div className="mb-3">
        ...
        <div className="h-1 rounded-full overflow-hidden" style={{background:"rgba(255,255,255,0.04)"}}><div className="h-full rounded-full transition-all duration-500" style={{width:`${(totalDone/8)*100}%`,background:"rgba(132,204,22,0.6)"}}/></div>
      </div>
      <div className="space-y-0.5">
        {lessons.map((l)=>{
          ...
          if(l.status==="active") return (
            <div key={l.num} className="flex items-center gap-2.5 px-2 py-1.5 rounded-lg" style={{background:"rgba(132,204,22,0.08)",border:"1px solid rgba(132,204,22,0.18)"}}>
              <span className="w-4 h-4 rounded-full flex items-center justify-center flex-shrink-0 relative" style={{background:"rgba(132,204,22,0.2)"}}><span className="w-1.5 h-1.5 rounded-full" style={{background:"#84CC16"}}/><span className="absolute inset-0 rounded-full animate-ping" style={{background:"rgba(132,204,22,0.3)"}}/></span>
              <span className="text-[10px] font-mono flex-shrink-0" style={{color:"rgba(132,204,22,0.6)"}}>L{l.num}</span>
              <span className="text-xs font-semibold truncate" style={{color:"#84CC16"}}>{l.title}</span>
            </div>
          );
          ...
          return (
            <Link key={l.num} href={l.href} className="flex items-center gap-2.5 px-2 py-1.5 rounded-lg hover:bg-white/3 transition-colors group">
              <span className="w-4 h-4 rounded-full flex items-center justify-center flex-shrink-0" style={{background:"rgba(255,255,255,0.05)"}}><svg width="6" height="8" viewBox="0 0 6 8"><path d="M1 4h4M3 1l2 3-2 3" stroke="rgba(255,255,255,0.3)" strokeWidth="1" strokeLinecap="round" fill="none"/></svg></span>
