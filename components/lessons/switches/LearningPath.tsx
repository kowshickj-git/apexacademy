...
interface Milestones {
  lessonRead: boolean;
  switchUsed: boolean;
  pushButtonUsed: boolean;
  openClosedUsed: boolean;
  homeLightUsed: boolean;
  arduinoUsed: boolean;
  quizPassed: boolean;
...
const lessons = [
  ...
  { id: "L13", title: "Switches",          href: "/electronics/switches",          active: true },
      ...
      <div className="mb-4">
        ...
        <div className="h-1.5 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.06)" }}>
          <div
            ...
            style={{ width: "65%", background: `linear-gradient(to right, ${CYAN}, #0891b2)` }}
          />
        </div>
        ...
        <div className="h-1 rounded-full overflow-hidden mt-1" style={{ background: "rgba(255,255,255,0.04)" }}>
          <div
            className="h-full rounded-full transition-all duration-700"
            style={{ width: `${(progress / total) * 100}%`, background: "rgba(6,182,212,0.5)" }}
          />
        </div>
      </div>
          ...
          if (l.active) {
            return (
              <div
                ...
                style={{ background: "rgba(6,182,212,0.10)", borderColor: "rgba(6,182,212,0.3)" }}
              >
                <div
                  className="w-6 h-6 rounded-lg border flex items-center justify-center text-[9px] font-mono font-bold"
                  style={{ background: "rgba(6,182,212,0.2)", borderColor: "rgba(6,182,212,0.4)", color: CYAN }}
                >
                  {l.id.slice(1)}
                </div>
                <span className="text-xs font-semibold" style={{ color: CYAN }}>{l.title}</span>
                <div className="ml-auto w-1.5 h-1.5 rounded-full animate-pulse" style={{ background: CYAN }} />
              </div>
            );
          }
          // done
