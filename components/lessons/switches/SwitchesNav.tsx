...
export default function SwitchesNav({ xp, milestones, onMenuClick }: Props) {
  ...
  const pipData = [
    ...
    { label: "Switch",     done: milestones.switchUsed },
    { label: "PushBtn",    done: milestones.pushButtonUsed },
    { label: "OpenClosed", done: milestones.openClosedUsed },
    { label: "HomeLight",  done: milestones.homeLightUsed },
    { label: "Arduino",    done: milestones.arduinoUsed },
    ...
    >
      <div className="max-w-5xl mx-auto px-3 h-14 flex items-center gap-2">
        ...
        <div className="hidden md:flex items-center gap-1.5 text-xs">
          ...
          <span className="font-medium" style={{ color: CYAN }}>Switches</span>
        </div>

        <span className="text-[10px] text-white/20 font-mono hidden lg:block">L13 of 36</span>
        ...
        <div className="hidden sm:flex items-center gap-1" title={`${pipData.filter(p => p.done).length}/7 milestones`}>
          {pipData.map((pip) => (
            <div
              ...
              style={{ background: pip.done ? CYAN : "rgba(255,255,255,0.1)" }}
              title={pip.label}
            />
          ))}
        </div>
        ...
        >
          <svg width="10" height="10" viewBox="0 0 12 12" fill={CYAN}>
            <polygon points="6,1 7.5,4.5 11,5 8.5,7.5 9,11 6,9.5 3,11 3.5,7.5 1,5 4.5,4.5" />
          </svg>
          <span className="text-xs font-bold tabular-nums" style={{ color: CYAN }}>{xp} XP</span>
        </div>
        ...
        <Link
          ...
          style={{ background: "rgba(6,182,212,0.09)", borderColor: "rgba(6,182,212,0.22)", color: CYAN }}
        >
      ...
      <div className="h-[2px] bg-white/4">
        <motion.div
          ...
          style={{ scaleX: scrollProgress / 100, background: `linear-gradient(to right, ${CYAN}, #0891b2)` }}
        />
      </div>
    </header>
  );
