...
export default function BreadboardsNav({ xp, milestones, onMenuClick }: Props) {
  ...
  const pipData = [
    ...
    { label: "Anatomy", done: milestones.anatomyUsed },
    { label: "Connections", done: milestones.connectionUsed },
    { label: "Circuit", done: milestones.circuitBuilt },
    ...
    >
      <div className="max-w-5xl mx-auto px-3 h-14 flex items-center gap-2">
        ...
        <div className="hidden md:flex items-center gap-1.5 text-xs">
          ...
          <span className="font-medium" style={{ color: "#14B8A6" }}>Breadboards</span>
        </div>

        <span className="text-[10px] text-white/20 font-mono hidden lg:block">L09 of 10</span>

        <div className="flex-1" />

        {/* Milestone pips */}
        <div className="hidden sm:flex items-center gap-1" title={`${pipData.filter(p => p.done).length}/5 milestones`}>
          {pipData.map((pip) => (
            <div
              key={pip.label}
              className="w-1.5 h-1.5 rounded-full transition-colors duration-500"
              style={{ background: pip.done ? "#14B8A6" : "rgba(255,255,255,0.1)" }}
              title={pip.label}
            />
          ))}
        </div>
        ...
        >
          <svg width="10" height="10" viewBox="0 0 12 12" fill="#14B8A6">
            <polygon points="6,1 7.5,4.5 11,5 8.5,7.5 9,11 6,9.5 3,11 3.5,7.5 1,5 4.5,4.5" />
          </svg>
          <span className="text-xs font-bold tabular-nums" style={{ color: "#14B8A6" }}>{xp} XP</span>
        </div>
        ...
        >
          ...
          <span className="hidden sm:inline">LEDs</span>
        </Link>
        ...
        <Link
          ...
          style={{ background: "rgba(20,184,166,0.09)", borderColor: "rgba(20,184,166,0.22)", color: "#14B8A6" }}
        >
      ...
      <div className="h-[2px] bg-white/4">
        <motion.div
          ...
          style={{ scaleX: scrollProgress / 100, background: "linear-gradient(to right, #14B8A6, #10B981)" }}
        />
      </div>
    </header>
  );
