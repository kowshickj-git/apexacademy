      ...
      <div className="max-w-2xl">
        ...
        >
          ...
          <p className="text-sm text-white/40 mb-4 leading-relaxed">
            Learn to measure voltage, current, and resistance. The multimeter is the most important tool in any electronics lab.
          </p>
          <Link
        ...
        <div className="flex items-center justify-between border-t border-white/5 pt-5">
          ...
          >
            ...
            <span className="group-hover:underline underline-offset-2">LEDs</span>
          </Link>

          <div className="flex items-center gap-2">
            <span className="text-[10px] text-white/15 font-mono">L09 of 10</span>
            <div className="flex gap-1">
              {Array.from({ length: 10 }).map((_, i) => (
                <div
                  key={i}
                  className="w-1.5 h-1.5 rounded-full"
                  style={{ background: i < 9 ? "rgba(20,184,166,0.5)" : "rgba(255,255,255,0.1)" }}
                />
              ))}
            </div>
          </div>
