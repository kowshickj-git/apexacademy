...
const COLORS=["#3B82F6","#60A5FA","#93C5FD","#1D4ED8","#BFDBFE","#2563EB","#60A5FA","#DBEAFE"];
...
export default function MOSFETComplete({ xp, onClose }: Props) {
  ...
  return (
    <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 z-[100] flex items-center justify-center p-4" style={{background:"rgba(5,5,7,0.92)",backdropFilter:"blur(16px)"}}>
      ...
      <motion.div initial={{scale:0.9,y:20,opacity:0}} animate={{scale:1,y:0,opacity:1}} exit={{scale:0.95,opacity:0}} transition={{type:"spring",damping:22,stiffness:200}} className="relative w-full max-w-sm rounded-3xl border border-white/10 overflow-hidden shadow-2xl" style={{background:"rgba(12,14,20,0.98)"}}>
        <div className="h-1.5" style={{background:"linear-gradient(to right, #3B82F6, #60A5FA)"}}/>
        <div className="p-6">
          ...
          <div className="rounded-2xl p-4 mb-4 text-center border" style={{borderColor:"rgba(59,130,246,0.25)",background:"rgba(59,130,246,0.06)"}}>
            ...
            <p className="text-3xl font-black" style={{background:"linear-gradient(to right,#3B82F6,#60A5FA)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>{xp} XP</p>
          </div>
          <div className="mb-4">
            <div className="flex justify-between text-[10px] text-white/30 mb-1 font-mono"><span>Course progress</span><span>18/20 = 90%</span></div>
            <div className="h-2 rounded-full overflow-hidden" style={{background:"rgba(255,255,255,0.06)"}}>
              <motion.div initial={{width:0}} animate={{width:"90%"}} transition={{duration:1,delay:0.5}} className="h-full rounded-full" style={{background:"linear-gradient(to right,#3B82F6,#60A5FA)"}}/>
            </div>
          </div>
          <div className="mb-4">
            <p className="text-[10px] text-white/25 uppercase tracking-widest mb-2 font-mono">Skills Unlocked</p>
            <div className="flex flex-wrap gap-2">
              {["MOSFET Switcher","PWM Master","Power Electronics","Quiz Champion"].map((s,i)=>(
                <motion.div key={s} initial={{opacity:0,scale:0.8}} animate={{opacity:1,scale:1}} transition={{delay:0.3+i*0.08}} className="px-3 py-1.5 rounded-full text-[10px] font-bold border" style={{borderColor:"rgba(59,130,246,0.3)",background:"rgba(59,130,246,0.1)",color:"#3B82F6"}}>✓ {s}</motion.div>
              ))}
            </div>
          </div>
          <Link href="/electronics/logic-gates" className="block p-4 rounded-2xl border mb-3 hover:opacity-90 transition-opacity group" style={{borderColor:"rgba(59,130,246,0.25)",background:"rgba(59,130,246,0.06)"}}>
            <p className="text-[9px] text-white/25 uppercase tracking-widest mb-0.5 font-mono">Next Lesson</p>
            <p className="text-sm font-bold text-white/85 group-hover:text-white transition-colors">Lesson 19: Logic Gates</p>
            <p className="text-[10px] text-white/35 mt-0.5">AND, OR, NOT, NAND — the building blocks of all digital logic →</p>
          </Link>
          <div className="flex gap-2">
            <Link href="/electronics/logic-gates" className="flex-1 py-2.5 rounded-xl font-bold text-sm text-center transition-all" style={{background:"rgba(59,130,246,0.15)",border:"1px solid rgba(59,130,246,0.35)",color:"#3B82F6"}}>Continue to Logic Gates →</Link>
