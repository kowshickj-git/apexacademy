...
export default function LEDsNav({ xp, milestones, onMenuClick }: Props) {
  ...
  const milestoneList = [milestones.lessonRead, milestones.polarityUsed, milestones.resistorUsed, milestones.rgbUsed, milestones.quizPassed];
    ...
    >
      <div className="max-w-5xl mx-auto px-3 h-14 flex items-center gap-2">
        ...
        <div className="hidden md:flex items-center gap-1.5 text-xs">
          ...
          <span className="text-secondary font-medium">LEDs</span>
        </div>

        <span className="text-[10px] text-white/20 font-mono hidden lg:block">L08 of 10</span>

        <div className="flex-1" />

        {/* Milestone pips */}
        <div className="hidden sm:flex items-center gap-1" title={`${completedCount}/5 milestones`}>
          {["Read", "Polarity", "Resistor", "RGB", "Quiz"].map((label, i) => (
            <div
              key={label}
      ...
      <div className="h-[2px] bg-white/4">
        <motion.div
          ...
          style={{ scaleX: scrollProgress / 100, background: "linear-gradient(to right, #F59E0B, #EF4444, #10B981)" }}
        />
      </div>
    </header>
  );
